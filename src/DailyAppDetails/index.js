...
import {fetchDailyInstall} from "../Action/daily";
...
const DailyAppDetails = (props) => {debugger
    ...
    const fetchData = () => {debugger
        if (props && props.location && props.location.state && props.location.state.applicationDetail && props.location.state.applicationDetail.appId) {
            const appData = props.location.state.applicationDetail
            setData(appData)
            avgDailyInstall(appData.appId, appData.Country)
            ageOfApplication(appData.released)
            getUpdatedDate(appData.updated)
        }
    };

    const avgDailyInstall = async (appId, Country) => {debugger
        setLoading(true)
        const res = await fetchDailyInstall({appId,Country});
        console.log("res",res)
        if (res.success) {
            setAppDetail({
...
export default DailyAppDetails;
